...
export default function NotificationsBell({ meId }) {
  ...
  useEffect(() => {
    ...
    setPanelStyle({ position: "fixed", top: rect.bottom + 4, left, width: panelWidth });
  }, [open]);
  ...
  return (
    <div className="relative" ref={ref}>
      <button ref={btnRef} onClick={() => { setOpen((v) => !v); if (!open) markAll(); }} className="relative p-2 rounded-lg hover:bg-secondary">
      ...
      {open && (
        <div style={panelStyle} className="z-50 glass border border-border rounded-2xl shadow-2xl overflow-hidden">
          <div className="px-3 py-2 text-xs text-muted-foreground border-b border-border">{t("notifications.title")}</div>
          <div className="max-h-80 overflow-y-auto">
            {items.length === 0 ? <div className="py-6 text-center text-sm text-muted-foreground">{t("notifications.empty")}</div> :
              items.map((n) => {
