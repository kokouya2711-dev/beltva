...
import { blockUser, unblockUser, muteUser, unmuteUser, blockExists, isMuted } from "@/lib/dm";
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 top-10 z-50 glass border border-border rounded-xl py-1 w-40">
          ...
          <button onClick={() => { setOpen(false); navigate("/report", { state: { target_type: "user", target_id: targetId, reported_id: targetId } }); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm hover:bg-secondary"><Flag className="w-4 h-4" /> {t("common.report")}</button>
        </div>
      )}
    </div>
  );
