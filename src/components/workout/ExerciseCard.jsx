    ...
    <div className="glass rounded-xl border border-border p-3">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-semibold text-sm truncate">{exercise.workout_type}</span>
        </div>
      ...
      <div className="space-y-1.5">
        {exercise.sets.map((set, si) => (
          <div key={si} className="flex items-center gap-1">
            ...
            <input type="number" inputMode="decimal" value={set.weight} onChange={e => onUpdateSet(index, si, "weight", e.target.value)} placeholder="kg" className="w-14 bg-secondary/60 border border-border rounded px-1.5 py-1 text-sm outline-none focus:border-primary" />
            <span className="text-[10px] text-muted-foreground">kg×</span>
            <input type="number" inputMode="decimal" value={set.reps} onChange={e => onUpdateSet(index, si, "reps", e.target.value)} placeholder="回" className="w-12 bg-secondary/60 border border-border rounded px-1.5 py-1 text-sm outline-none focus:border-primary" />
            <span className="text-[10px] text-muted-foreground">回</span>
            <button onClick={() => onCompleteSet(index, si)} className="p-1 rounded hover:bg-primary/20 text-primary shrink-0" title="セット完了→レスト">
