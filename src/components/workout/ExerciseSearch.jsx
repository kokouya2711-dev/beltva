...
import { useTWorkout, useTBodyPart } from "@/lib/i18nHelpers";
...
export default function ExerciseSearch({ onSelect, onClose, exclude = [] }) {
  ...
  return (
    <div className="fixed inset-0 z-[80] bg-black/85 backdrop-blur-sm flex items-end md:items-center justify-center" onClick={onClose}>
      <div className="w-full md:max-w-md bg-card border border-border rounded-t-2xl md:rounded-2xl p-4 max-h-[88vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-3">
          <h3 className="font-bold">{t("ex.search")}</h3>
        ...
        {!q && (
          <div className="flex gap-1.5 mb-3 overflow-x-auto no-scrollbar">
            {BODY_PARTS.map(bp => (
              <button key={bp} onClick={() => setBodyPart(bp)} className={`shrink-0 text-xs px-2.5 py-1 rounded-full border ${bodyPart === bp ? "border-primary bg-primary/15 text-primary" : "border-border text-muted-foreground"}`}>{tBodyPart(bp)}</button>
            ))}
          </div>
        )}
        <div className="space-y-1 mb-3">
          ...
          {filtered.length === 0 && <div className="text-center text-sm text-muted-foreground py-4">{t("ex.noResults")}</div>}
        </div>
        <button onClick={() => setShowAdd(v => !v)} className="w-full flex items-center justify-center gap-1.5 text-sm text-primary border border-dashed border-primary/30 rounded-lg py-2.5 hover:bg-primary/5">
          <UserPlus className="w-4 h-4" /> {t("ex.addCustom")}
        </button>
        {showAdd && (
          <div className="mt-2 p-3 rounded-lg border border-border space-y-2">
            <input value={newName} onChange={e => setNewName(e.target.value)} placeholder={t("ex.namePlaceholder")} className="w-full bg-secondary/60 border border-border rounded px-3 py-2 text-sm outline-none focus:border-primary" />
            <select value={newPart} onChange={e => setNewPart(e.target.value)} className="w-full bg-secondary/60 border border-border rounded px-3 py-2 text-sm outline-none focus:border-primary">
              {BODY_PARTS.map(bp => <option key={bp} value={bp}>{tBodyPart(bp)}</option>)}
            </select>
            <button onClick={addCustom} className="w-full bg-primary text-primary-foreground text-sm font-semibold py-2 rounded-lg">{t("ex.add")}</button>
          </div>
        )}
      </div>
    </div>
