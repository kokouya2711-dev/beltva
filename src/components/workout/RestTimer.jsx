...
export default function RestTimer({ onClose }) {
  ...
  function fmtPreset(s) {
    if (s >= 60 && s % 60 === 0) return `${s / 60}${t("common.min")}`;
    if (s >= 60) return `${Math.floor(s / 60)}${t("common.min")}${s % 60}${t("common.sec")}`;
    return `${s}${t("common.sec")}`;
  }
  ...
  return (
    <div className="fixed bottom-24 md:bottom-6 left-1/2 -translate-x-1/2 z-[65] glass border border-primary/30 rounded-2xl p-4 shadow-2xl min-w-[300px] max-w-[360px]">
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2"><Timer className="w-4 h-4 text-primary" /><span className="font-bold text-sm">{t("rest.title")}</span></div>
      ...
      {restRemaining > 0 ? (
        <div className="text-center">
          ...
          <div className="flex gap-2 mt-2 justify-center">
            <button onClick={() => restRunning ? pauseRest() : startRest(restRemaining)} className="text-xs px-3 py-1 rounded-lg bg-secondary border border-border">{restRunning ? t("rest.pause") : t("rest.resume")}</button>
            <button onClick={stopRest} className="text-xs px-3 py-1 rounded-lg bg-secondary border border-border">{t("rest.reset")}</button>
          </div>
        </div>
      ) : (
        <div className="space-y-2">
          ...
          <button onClick={() => setShowAdd(v => !v)} className="w-full flex items-center justify-center gap-1 text-xs text-primary hover:underline">
            <Plus className="w-3 h-3" /> {t("rest.addCustom")}
          </button>
          {showAdd && (
            <div className="flex gap-1.5 items-center">
              <input type="number" value={newMin} onChange={e => setNewMin(e.target.value)} placeholder={t("common.min")} className="w-16 bg-secondary/60 border border-border rounded px-2 py-1 text-sm outline-none focus:border-primary" />
              <span className="text-xs text-muted-foreground">{t("common.min")}</span>
              <input type="number" value={newSec} onChange={e => setNewSec(e.target.value)} placeholder={t("common.sec")} className="w-16 bg-secondary/60 border border-border rounded px-2 py-1 text-sm outline-none focus:border-primary" />
              <span className="text-xs text-muted-foreground">{t("common.sec")}</span>
              <button onClick={addCustom} className="bg-primary text-primary-foreground text-sm px-3 py-1 rounded-lg">{t("ex.add")}</button>
            </div>
          )}
        </div>
      )}
