...
import { ArrowLeft, HeartPulse, Dumbbell, Trophy, Users } from "lucide-react";
import { useT, useI18n } from "@/lib/i18n";
import { useScrollLock } from "@/hooks/useScrollLock";
import { purposeLabel } from "@/lib/i18nPurposeFilter";

// 登録フロー内の目的選択ステップ
// プログレスバーは7番目(index 6)をハイライト
const OPTIONS = [
  { key: "health", Icon: HeartPulse },
  { key: "bodymake", Icon: Dumbbell },
  { key: "contest", Icon: Trophy },
  { key: "friends", Icon: Users },
];

export default function PurposeStep({ onBack, onContinue, loading, initialValue = "" }) {
  const t = useT();
  const { lang } = useI18n();
  useScrollLock();
  ...
  return (
    <div className="relative min-h-[100dvh] flex flex-col bg-background overflow-hidden">
      ...
      <div className="flex-1 flex flex-col px-6 pt-10">
        <h2 className="text-center text-xl font-bold text-foreground">{t("auth.purposeTitle")}</h2>
          ...
          {OPTIONS.map(({ key, Icon }) => {
            ...
            return (
              ...
              >
                ...
                <span className={`flex-1 text-left text-base font-semibold ${active ? "text-foreground" : "text-foreground"}`}>
                  {purposeLabel(lang, key)}
                </span>
