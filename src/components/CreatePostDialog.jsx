...
export default function CreatePostDialog({ onClose, onSaved }) {
  ...
  async function handleFileSelect(e) {
    ...
    try {
      ...
      setMediaUrls(prev => {
        let next = [...prev, ...urls.map(u => u.file_url)];
        const videoIdx = next.findIndex(isVideoUrl);
        if (videoIdx >= 0) next = next.filter((u, i) => i === videoIdx || !isVideoUrl(u));
            ...
            <div className="mt-1.5 grid grid-cols-3 gap-1.5">
              {mediaUrls.map((url, i) => (
                <div key={i} className="relative aspect-square rounded-lg overflow-hidden">
                  ...
                  <button onClick={() => setMediaUrls(prev => prev.filter((_, j) => j !== i))} className="absolute top-1 right-1 bg-black/70 rounded-full p-0.5">
                    <X className="w-3 h-3 text-white" />
                  </button>
                </div>
              ))}
              {mediaUrls.length < 9 && (
                <button onClick={() => fileRef.current?.click()} disabled={uploading} className="aspect-square border border-dashed border-border rounded-lg flex items-center justify-center text-muted-foreground hover:border-primary hover:text-primary transition">
