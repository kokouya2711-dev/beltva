...
import { Radio, MapPin, Flame, Eye, Heart, Trophy } from "lucide-react";
    ...
    <div className="relative glass rounded-2xl overflow-hidden border border-border hover:border-primary/40 transition group">
        ...
        <div className="absolute top-3 left-3 flex items-center gap-2">
          {live ? (
            <span className="flex items-center gap-1.5 bg-red-500 text-white text-[11px] font-bold px-2 py-1 rounded-md">
              <span className="w-1.5 h-1.5 rounded-full bg-white live-dot" /> {t("live.training")}
            </span>
          ) : (
            <span className="bg-secondary text-muted-foreground text-[11px] font-bold px-2 py-1 rounded-md">{t("live.end")}</span>
          )}
          {live && (
      ...
      <div className="px-4 pt-7 pb-4">
        <div className="flex items-center gap-2 text-xs text-muted-foreground mb-2">
          ...
          {session.location_name || t("live.unknownLocation")}
          {session.lat && (
        ...
        <div className="flex items-center gap-2 mt-4">
          {live && (
            ...
            >
              <Heart className="w-4 h-4" /> {t("live.cheer")}
            </button>
          )}
          {isOwner && live ? (
            <button
            ...
            >
              <Radio className="w-4 h-4" /> {t("live.watch")}
            </button>
          )}
        </div>
      </div>
